var Looper = (function () {
    ...
    var conn = function (env, url, input) {
        this.env = env;
        this.input = input;
        this.chatting = true;
        this.url = url;
...
(function (opt) {
    ...
    var initStyle = function () {
        ...
        if (!opt.mobile) {
            if (opt.fontSize) {
                ...
                var style = $.currentStyle(stage);
                var lineHeight = style && style.getPropertyValue('line-height');
                if (lineHeight) {
                    try {
                        lineHeight = parseInt(lineHeight);
                        if (lineHeight <= opt.fontSize) {
                            stage.style.lineHeight = (opt.fontSize + 5) + 'px';
                        }
                    } catch (e) {
        ...
        var mobileColor = opt.mobileChatColor || opt.compId >= 20000000 && '#ff6e69' || '#00a553';
        var mobileTpl = '.mobile .inMsg .m-photo-head{background-color:{0}} .mobile .inMsg{border-color:{0}} ' +
            '.mobile .inMsg .msg-cursor{border-right-color:{0}} .mobile .inMsg .msg-cursor-inner {border-right-color:#FFF}';
        var cssStyle = $.tpl(mobileTpl, [mobileColor]);
    ...
    $.onReady(function () {
        ...
        var backBtn = $('back');
        backBtn && (backBtn.onclick = function () {
    ...
    var initChat = function () {
        ...
        if (opt.meeting) {
            ...
            popMessage({type: 'chatBegin', data: {}});
    ...
    var receivedMessage = {};
    var loadedMessage = {};
    var looper = new Looper(env, url, $('message'));
    ...
    var doAppend = function (msg, clz) {
        ...
        if (conf.mobile) {
            ...
            if (notFirst == 1) {
                setTimeout(function () {
                    tar.style.marginTop = (screen.height - 130 - tar.offsetHeight) + 'px';
        ...
        browserImage: (function () {
            ...
            var initPreview = function () {
                ...
                previewImg.onload = function () {
                    this.style.marginTop = (screen.height - this.height) / 2 + 'px';
