...
var resizeLayout = function () {
    var ad1 = $('ad1'), stage = $('stage'), ad2 = $('ad2'), desc = $('descInfo'), tool = $('tool'), inputer = $('inputer');
    var main = $('main');
    if (!stage && !main)
        return;
...
$.onReady(function () {
    if (conf.mobile != 1) {
        var ad = {logo: $('header'), ad1: $('ad1'), ad2: $('ad2'), card: conf.cardImage && conf.chatId > 0 && $('ad2')};
        ...
        for (var v in ad) {
            ...
            if (ad[v]) {
                if (v == 'ad2' && ad['ad1'] || v == 'logo' && conf.command == 'forceChat')
                    continue;
                var img = fullurl(conf[v + 'Image']), tar = conf[v + 'Target'], re = /.*\.swf/ig, isflash = re.test(img);
                if (v != 'logo')
